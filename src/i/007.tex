...
\begin{header}
\swa{A.K. Aitim}{BUILDING A HIGH-QUALITY ANNOTATED CORPUS FOR KAZAKH NLP: A PIPELINE APPROACH}
...
\corrauthor{Сorresponding author: a.aitim@iitu.edu.kz}
\end{affil}
...
baseline model performance (POS, NER, parsing) to demon\-strate utility.
...
annotation process, including maintain\-ing consistency, assessing
...
\begin{header}
ҚАЗАҚША NLP ҮШІН ЖОҒАРЫ САПАЛЫ АННОТАЦИЯЛЫҚ КОРПУС ҚҰРУ: ҚҰБЫРЛЫҚ ТӘСІЛ
...
\begin{header}
ПОСТРОЕНИЕ ВЫСОКОКАЧЕСТВЕННОГО АННОТИРОВАННОГО КОРПУСА ДЛЯ КАЗАХСКОГО NLP: ПАЙПЛАЙН-ПОДХОД

A.K.Айтим
\end{header}

\begin{affil}
Международный Университет Информационных Технологий, Алматы, Казакстан,
...
Total sentences                    & \textasciitilde{}350,000                    \\
Total tokens                       & \textasciitilde{}6.2 million                \\
Fully verified annotated sentences & \textasciitilde{}150,000                    \\
Unique word forms                  & \textasciitilde{}210,000                    \\
...
Documents           & N\_DOCS        & After de-duplication            \\
Sentences           & N\_SENTS       & Sentence splitter TOOL          \\
Tokens              & N\_TOK         & Word tokenizer TOOL             \\
Unique types        & N\_TYPES       & Lowercased, punctuation removed \\
Verified sentences  & N\_VERIFIED    & Double-blind + adjudicated      \\
Domains (K)         & K              & 5                               \\
Avg tokens/sentence & MEAN\_LEN ± SD & Trimmed at P99                  
\end{longtblr}
...
\tcap{Table 3 - Inter-annotator agreement}
\begin{longtblr}[
  label = none,
  entry = none,
]{
  width = \linewidth,
  colspec = {Q[119]Q[75]Q[158]Q[146]Q[148]Q[173]Q[110]},
  cells = {c},
  cells = {font = \small},
  cell{6}{1} = {c=7}{0.928\linewidth},
  cell{6}{1} = {halign=l},
  hlines,
  vlines,
}
\textbf{Layer}                                   & \textbf{Units} & \textbf{Metric}   & \textbf{Value} & \textbf{n}  & \textbf{Description} & \textbf{Score} \\
POS                                              & token          & Cohen’s κ         & K\_POS         & N\_TOK\_IAA & tagset SIZE          & 0.94           \\
Morphology                                       & token          & Cohen’s κ         & K\_MORPH       & N\_TOK\_IAA & features list        & 0.91           \\
NER                                              & spans          & micro-F1 (strict) & F1\_NER        & N\_SPANS    & entity set           & 0.89           \\
Parsing                                          & tokens         & LAS / UAS         & LAS/UAS        & N\_ARCS     & punctuation policy   & 88.2 / 91.5    \\
\textit{Authors’ annotated Kazakh corpus (2025)} &                &                   &                &             &                      &                
\end{longtblr}
...
\tcap{Table 4 - Model performance}
\begin{longtblr}[
  label = none,
  entry = none,
]{
  width = \linewidth,
  colspec = {Q[346]Q[277]Q[156]Q[160]},
  cells = {font = \small},
  row{even} = {c},
  row{1} = {c},
  row{3} = {c},
  cell{5}{1} = {c=4}{0.939\linewidth},
  hlines,
  vlines,
}
\textbf{Task}                                                                                            & \textbf{Model}     & \textbf{Metric} & \textbf{Score} \\
...
\textit{Comparative analysis based on UD Kazakh-KTB (Tyers \& Washington, 2015) and KazNERD (LREC 2022)} &                    &                 &                
\end{longtblr}
...
\tcap{Table 5 - Positioning against representative Kazakh resources}
\begin{longtblr}[
  label = none,
  entry = none,
]{
  width = \linewidth,
  colspec = {Q[175]Q[94]Q[169]Q[121]Q[106]Q[154]Q[113]},
  cells = {c},
  cells = {font = \small},
  hlines,
  vlines,
}
\textbf{Resource}   & \textbf{Tokens} & \textbf{Layers}     & \textbf{Domains} & \textbf{IAA reported} & \textbf{Baselines reported} & \textbf{Public scripts} \\
UD Kazakh-KTB       & ≈200,000        & POS, UD             & News/General     & Yes                   & Yes (parsing)               & Yes                     \\
KazNERD (LREC 2022) & ≈1,300,000      & NER                 & TV news/media    & Yes                   & Yes (NER)                   & Yes                     \\
...
\fig[0.45\textwidth]{i/image42}[Fig.1 - CoNLL-U file loader\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]

\fig[0.7\textwidth]{i/image43}[Fig.2 - POS pre-annotation with KazBERT\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]
...
\fig[0.62\textwidth]{i/image44}[Fig.3 - Verifiable pipeline kernel\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]
...
\fig[0.62\textwidth]{i/image45}[Fig.4 - Sentence lengths by domain\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]
...
\fig[0.62\textwidth]{i/image46}[Fig.5 - Token and sentence count per document\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]
...
\fig[0.3\textwidth]{i/image47}[Fig.6 - Morphological feature distribution\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]
...
\fig[0.6\textwidth]{i/image48}[Fig.7 - Model accuracy across NLP tasks\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]
...
\fig[0.6\textwidth]{i/image49}[Fig.8 - Multi-line chart: accuracy trends across training epochs\\\normalfont{\emph{Authors' experiment using QNLP pipeline (2025) {[}1{]}}}]
...
6. Aitim A.K., Satybaldiyeva R.Zh., Wojcik W. The construction of the
...
8. Houlsby N., Giurgiu A., Jastrzebski S., Morrone B., de Laroussilhe
