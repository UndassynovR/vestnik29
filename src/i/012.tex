...
{\bfseries USER AUTHENTICATION IN MOBILE LEARNING: BEHAVIORAL ANALYSIS WITH
GYROSCOPE AND ACCELEROMETER DATA}

{\bfseries B.B.
Serbin}{\bfseries ,
A.M.
Kassenkhan}{\bfseries \envelope 
, R.K.
Uskenbayeva}{\bfseries ,
Zh.B.
Kalpeyeva}
...
Keywords: smartphone, mobile application, behavioral model,
authentication, gyroscope, accelerometer, security, education.

{\bfseries МОБИЛЬДІ ОҚЫТУДАҒЫ ПАЙДАЛАНУШЫЛАРДЫ АУТЕНТИФИКАЦИЯЛАУ: ГИРОСКОП
ПЕН АКСЕЛЕРОМЕТР ДЕРЕКТЕРІН ПАЙДАЛАНА ОТЫРЫП, МІНЕЗ-ҚҰЛЫҚТЫ ТАЛДАУ}

{\bfseries В.В. Сербин, А.М. Қасенхан\envelope , Ж.Б. Кальпеева,
Р.К.Ускенбаева}

\emph{Satbayev University, Алматы, Қазақстан,}
...
{\bfseries АУТЕНТИФИКАЦИЯ В МОБИЛЬНОМ}

{\bfseries ОБУЧЕНИИ: ПОВЕДЕНЧЕСКИЙ АНАЛИЗ С ИСПОЛЬЗОВАНИЕМ ДАННЫХ ГИРОСКОПА
И АКСЕЛЕРОМЕТРА}

\texorpdfstring{В.В. Сербин, А.М. Қасенхан\envelope ,
Ж.Б. Кальпеева,
Р.К.Ускенбаева}{В.В. Сербин, А.М. Қасенхан\envelope , Ж.Б. Кальпеева, Р.К.Ускенбаева}
...
{\bfseries Fig№ 1 - Annual number of publications on "behavioral
authentication" and "mobile learning"}
...
k\tsb{g} - gyroscope correlation coefficient;

k\tsb{a} - accelerometer correlation coefficient.
...
\fig{i2/image57}{}
\(k_{g} = \frac{1}{3}(k_{x} + k_{y} + k_{z})\) (2)

where,

k\tsb{x} - correlation coefficient of the
gyroscope' s X-axis;

k\tsb{y} - correlation coefficient of the
gyroscope' s Y-axis;

k\tsb{z} - correlation coefficient of the
gyroscope' s Z-axis.

Calculated from Equation (3) is the correlation of each parameter:

\(k_{x} = \frac{x_{avg} \ast x}{\sigma_{x_{2}}\sigma_{x_{1}}}\) (3)

This is an example of a calculation of a correlation of a X-axis as
follows:

x\tsb{avg} - the average value of all X-axis readings in a
single test;

x - the initial X-axis value during testing;

σx\tsb{1} - the standard deviation of the first test value;

σx\tsb{2} - the standard deviation of the second test value.
...
{\bfseries Table 1-Decision making}
...
%% \bottomrule\noalign{}
%% \endlastfoot
%% {\bfseries Correlation coefficient range} & {\bfseries Decision} \\
%% If (k \textgreater{} 0.5) & User identified \\
%% If (k <{} 0.5) and (k \textgreater{} 0) & User identity is
%% questionable \\
...
{\bfseries Table 2-Condition 1. When the phone is on the surface}
...
%% \bottomrule\noalign{}
%% \endlastfoot
%% {\bfseries Group} & {\bfseries X\tsb{avr}} &
%% {\bfseries Y\tsb{avr}} & {\bfseries Z\tsb{avr}} &
%% {\bfseries A\tsb{avr}} \\
...
{\bfseries Table 3-State 2. When a person holds the phone stationary (0
km/h)}
...
{\bfseries Table 4 - State 3. When a person is sitting}
...
{\bfseries Table 5 - Condition 4. When a person lies}

%% \begin{longtable}[]{@{}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1988}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2004}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2006}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1995}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2007}}@{}}
%% \toprule\noalign{}
%% \endhead
%% \bottomrule\noalign{}
%% \endlastfoot
...
{\bfseries Table 6 - Condition 5. When a person walks at an average speed
of 4-5 km/h.}

%% \begin{longtable}[]{@{}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2000}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1997}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2000}}
%%   >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2003}}
...
\fig{i2/image60}{}\fig{i2/image60}{}=
-5,264 -- 3,44 = 1,06;

2) Variance =
\fig{i2/image61}{}\fig{i2/image61}{}
= 1.06\^{}2=1.13;
...
\fig{i2/image62}{}\(k_{x} = \frac{x_{avg} \ast x}{\sigma_{x_{2}} \ast \sigma_{x_{1}}} = 0,757535214\)
...
\fig{i2/image63}{}\fig{i2/image63}{}\(k_{a} = \left( \frac{A_{avg} \ast A}{\sigma_{A_{2}} \ast \sigma_{A_{1}}} \right) = 0,524778876\)
