...
% When in figs environment, #1 is the minipage width, image fills it
\newcommand{\galleryimg}[3]{%
  \begin{minipage}[t]{#1}
    \centering
    \includegraphics[width=\textwidth]{media/#2}%
    \if\relax\detokenize{#3}\relax
      % no caption
    \else
      \par\vspace{1ex}\small\textbf{#3}%
    \fi
  \end{minipage}%
}
...
% Command to set height: \figheight{2cm}
\newcommand{\setheight}[1]{\gdef\figheight{#1}}

% Command to clear height
\newcommand{\clearheight}{\gdef\figheight{}}

% Main command: \fig[width]{path}[caption]
% To use height, call \setheight{2cm} before \fig
\NewDocumentCommand{\fig}{O{0.8\textwidth} m o}{%
  \ifinfigs
    \IfNoValueTF{#3}{%
      \galleryimgwithopts{#1}{#2}{}%
    }{%
      \galleryimgwithopts{#1}{#2}{#3}%
    }%
  \else
    \begin{figure}[H]
      \centering
    ...
    \end{figure}%
  \fi
  \clearheight% reset after each use
}

% Helper to make image with optional height
\newcommand{\makeimgwithopts}[2]{%
  \ifx\figheight\empty
    \includegraphics[width=#1]{media/#2}%
  \else
    \includegraphics[width=#1,height=\figheight,keepaspectratio=false]{media/#2}%
  \fi
}

% Helper for gallery images
\newcommand{\galleryimgwithopts}[3]{%
  \begin{minipage}[t]{#1}
    \centering
    \ifx\figheight\empty
      \includegraphics[width=\textwidth]{media/#2}%
    \else
      \includegraphics[width=\textwidth,height=\figheight,keepaspectratio=false]{media/#2}%
    \fi
    \if\relax\detokenize{#3}\relax
      % no caption
    \else
...
% \begin{figs}
%   \fig[0.45\textwidth]{i/image2}[1-сурет. Caption text]
%   \fig[0.45\textwidth]{i/image3}[2-сурет. Caption text]
% \end{figs}
%
...
%
% Single figure without caption (default width):
% \fig{path/to/image}
%
% Mix and match:
% \begin{figs}
%   \fig{i/image2}[Has caption]
%   \fig{i/image3}
% \end{figs}
