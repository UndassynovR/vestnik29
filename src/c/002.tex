...
\begin{header}
\swa{R.Vardanyan, E.Kozhevnikova, N.Akatyev}{MOLECULAR-LEVEL ASSESSMENT OF THE CORROSION INHIBITION PROPERTIES OF ONO-TYPE SCHIFF BASES USING DFT CALCULATIONS AND MOLECULAR DYNAMICS SIMULATIONS}
...
\corrauthor{Corresponding author: nikolay.akatyev@wku.edu.kz}
\end{affil}
...
\begin{header}
ТФТ-ЕСЕПТЕУЛЕРІ МЕН МОЛЕКУЛАЛЫҚ ДИНАМИКАЛЫҚ ҮЛГІЛЕУ АРҚЫЛЫ ONO-ТИПТІ ШИФФ НЕГІЗДЕРІНІҢ КОРРОЗИЯҒА ҚАРСЫ ҚАСИЕТТЕРІН МОЛЕКУЛАЛЫҚ ДЕҢГЕЙДЕ БАҒАЛАУ
...
сон\-дай-ақ 3,5-ди-\emph{трет}-бутилсалицил альдегидтен (SB2) синтезделген
...
кванттық-химиялық есептеулер, моле\-кулалық-динамикалық (МД) модельдеу,
органикалық коррозия ингибиторлары, Шифф негіздері, кванттық-химиялық
дескрипторлар, адсорбция.

\begin{header}
ОЦЕНКА АНТИКОРРОЗИОННЫХ СВОЙСТВ ОСНОВАНИЙ ШИФФА ONO-ТИПА НА МОЛЕКУЛЯРНОМ УРОВНЕ С ИСПОЛЬЗОВАНИЕМ DFT-РАСЧЕТОВ И МОЛЕКУЛЯРНО-ДИНАМИЧЕСКОГО МОДЕЛИРОВАНИЯ
...
антикоррозионных свой\-ств двух оснований Шиффа ONO-типа с
использованием методов теории функционала плотности (ТФП) и
молекулярной динамики (МД).  Основания Шиффа, синтезированные из
\emph{о}-аминофенола, \emph{о}-гидро\-ксиацетофенона (SB1) и
3,5-ди-\emph{трет}-бутилсалицилового альдегида (SB2), были исследованы
с точки зрения их электронной структуры, характеристик реакционной
способности и поведения при адсорбции на поверхности Fe(110). Расчеты
методом DFT на уровне B3LYP/6-311+G(d,p) в водной фазе выявили
ключевые различия в молекулярных свойствах, таких как энергии
граничных орбиталей, дипольный момент, энергетическая щель и
распределение заряда.  Анализ распределения зарядов по Малликену и
функций Фукуи выявили первичные реакционные центры, подчеркнув роль
гетероатомов в поверхностных адсорбционных взаимодействиях. Для
сравнения способности ингибиторов отдавать и принимать электроны также
были оценены глобальные дескрипторы реакционной
способности. Дополнительное МД-моделирование позволило получить
представление о влиянии геометрии молекул и заместителей на процесс
адсорбции. SB1 показал более благоприятную пространственную ориентацию
и более сильное поверхностное взаимодействие, в то время как
стерические препятствия из-за объёмных \emph{трет}-бутильных групп в
SB2 ограничивали эффективность его адсорбции. Такой комплексный
вычислительный подход обеспечивает детальное понимание взаимосвязи
структуры и активности ингибиторов коррозии и способствует
рациональному проектированию материалов нового поколения для защиты
металлов.

{\bfseries Ключевые слова:} теория функционала плотности (ТФП),
квантово-химические расчеты, моле\-кулярно-динамическое (МД)
...
\fig[0.5\textwidth]{c/image2}[Fig.1 - Chemical structure with atomic enumeration of
investigated ONO-type Schiff bases]

\begin{multicols}{2}
{\bfseries Materials and methods.} All quantum chemical calculations
were performed on a desktop PC with Windows 11, a 12th-generation
Intel(R) Core(TM) i7-12700H 2.30 GHz, 16 GB RAM, using GAMESS software
[9]. Initially, full geometry optimization was achieved using the
Molecular Mechanics (MM+) force field. The results from MM+ were then
used as input and re-optimized using the semi-empirical PM3 method to
obtain the equilibrium geometry. Final geometry optimization and
quantum chemical parameters were obtained by density functional theory
(DFT) calculations.

To calculate optimized geometrical structure, atomic charges, and
energies, the B3LYP functional with the 6-311+G(d,p) basis set was
used. All DFT calculations were carried out in the aqueous phase
because electrochemical corrosion occurs in aqueous media. The solvent
(\(H_{2}O\)) was incorporated via the Conductor-like Screening Model
(COSMO) [10].
...
(\(E_{HOMO}\)), the energy of the lowest unoccupied molecular orbital
(\(E_{LUMO}\)), the energy gap (\(\Delta E_{gap}\)), ionization
potential (IP), electron affinity (EA), electronegativity (\(\chi\)),
global hardness (\(\eta\)), softness (\(\sigma\)), electrophilicity
(\(\omega\)), nucleophilicity (\(\varepsilon\)), back-donation energy
(\(E_{b-d}\)), total negative charge (TNC, \(TNC/n\), where \(n\) is
the total number of atoms in the molecule), and the fraction of
transferred electrons (\(\Delta N\)), considered according to the
...
\begin{equation}
\Delta E_{b-d}=-\frac{\eta}{4}
\end{equation}

\begin{equation}
\Delta N=\frac{\chi_{Fe}-\chi_{inhibitor}}{2(\eta_{Fe}-\eta_{inhibitor})}
\end{equation}

where \(\chi_{Fe}\) and \(\chi_{inh}\) are the absolute
electronegativities of the iron atom and the inhibitor molecule,
respectively; \(\eta_{Fe}\) and \(\eta_{inh}\) the absolute hardness
of the iron atom and the inhibitor molecule, respectively. The
electronegativity of iron is 7 eV, and the total hardness is 0 eV.

According to Koopman's theorem, the HOMO energy is related to IP while
the LUMO energy is related to EA. Electronegativity (\(\chi\)) and
global hardness (\(\eta\)) were evaluated based on the finite
difference approximation as linear combinations of the calculated IP
and EA. The global softness (\(\sigma\)) is the reciprocal of the
global hardness. The global electrophilicity index (\(\omega\))
describes the stabilization of a molecule after the acquisition of an
additional number of electrons. The nucleophilicity index
(\(\varepsilon\)) is the reciprocal of the global electrophilicity
index [12].
...
\begin{equation}
E_{adsorption}=E_{total}-(E_{Fe(110)}+E_{inhibitor})
\end{equation}

where \(E_{adsorption}\) is the adsorption energy (or interaction
energy) corresponding to the amount of energy released or absorbed as
1 mole of adsorbate molecules is adsorbed on the adsorbent,
\(E_{total}\) is the total energy of the whole system (surface
\(Fe(110)\) + inhibitor molecule), \(E_{Fe(110)}\) is the energy of
the metal, and \(E_{inhibitor}\) is the energy of the inhibitor
molecule.  The binding energy is regarded as the reciprocal of the
adsorption energy of the inhibitor molecule as follows (12):

\begin{equation}
E_{binding}=-E_{adsorption}
\end{equation}

where \(E_{binding}\) is the binding energy.
...
surfaces, can be effectively analyzed using Frontier Molecular
Orbitals (FMOs) and Molecular Electrostatic Potential (MEP). Optimized
geometry, HOMO, LUMO energies and density distribution, energy gap
\(\Delta E_{gap}\), and MEP for SB1 and SB2 molecules calculated using
the DFT/B3LYP/6-311+G(d,p) method are presented in Figure 2. The
positively charged lobe is indicated by the blue color, and the
negatively charged lobe is indicated by the red color.
\end{multicols}

\fig[0.5\textwidth]{c/image10}[Fig.2 - HOMO, LUMO energies and density
distribution, energy gap (ΔE\emph{\tsb{gap}}), and MEP for SB1 (A) and
SB2 (B) mole­cules calculated using DFT/B3LYP/6-311+G(d,p) method]
...
\(f^+_k=P_k(N+1)-P_k(N)\) (For nucleophilic attack)

\(f^-_k=P_k(N)-P_k(N-1)\) (For electrophilic attack)

\(f^0_k=\frac{P_k(N+1)-P_k(N-1)}{2}\) (For radical attack)
...
\tcap{Table 1 - Mulliken atomic charges and Fukui functions calculated using DFT/B3LYP/6-311+G(d,p) for ONO-type Schiff bases}
\begin{longtblr}[
  label = none,
  entry = none,
]{
...
Atoms*       & P\textit{k}(N) & P\textit{k}(N+1) & P\textit{k}(N-1) &                 &                 &                 \\
\textbf{SB1} &                &                  &                  &                 &                 &                 \\
...
C(6)         & -0.1900        & -0.0480          & -0.2005          & \textbf{0.1420} & 0.0105          & 0.0762          \\
C(7)         & 0.2863         & 0.1802           & 0.3241           & -0.1061         & -0.0378         & -0.0719         \\
N(8)         & -0.5385        & -0.5096          & -0.5697          & 0.0289          & \textbf{0.0312} & 0.0300          \\
...
C(14)        & 0.1784         & 0.2870           & 0.1939           & 0.1086          & -0.0155         & \textbf{0.0465} \\
...
\textbf{SB2} &                &                  &                  &                 &                 &                 \\
C(1)         & 0.1309         & 0.3687           & 0.2464           & \textbf{0.2378} & -0.1155         & \textbf{0.1766} \\
...
N(8)         & -0.3201        & -0.5249          & -0.5653          & -0.2048         & \textbf{0.2462} & -0.2255         \\
...
\end{longtblr}
\tcap{\normalfont{* - \emph{atomic enumeration} \emph{is in accordance with Fig.1.}}}
...
\tcap{Table 2 - Molecular reactivity descriptors calculated for ONO-type Schiff bases calculated using DFT/B3LYP/6-311+G(d,p) in aqueous phase}
\begin{longtblr}[
  label = none,
  entry = none,
]{
...
\textbf{Descriptor}                                                      & \textbf{SB1} & \textbf{SB2} \\
Total energy (E\textit{tot}), a.u.                                       & -746.25      & -1021.14     \\
Dipole moment (μ), D                                                     & 2.8142       & 1.7517       \\
Ionization energy (IP), eV                                               & 5.6900       & 9.4940       \\
Electron affinity (EA), eV                                               & 1.6480       & 4.2780       \\
Electronegativity (\textit{}), eV                                       & 3.6690       & 6.8860       \\
Chemical hardness (\textit{η}), eV                                       & 2.0210       & 2.6080       \\
Chemical softness (σ), eV                                                & 0.4948       & 0.3834       \\
Electrophilicity index (ω), eV                                           & 6.7308       & 23.7085      \\
Nucleophilicity index (\textit{ε}), eV                                   & 0.1486       & 0.0422       \\
Back-donation energy (E\textit{b-d}, eV)                                 & -0.5053      & -0.6520      \\
Fraction of transferred electrons (Δ\textit{N})                          & -0.8241      & -0.0219      \\
TNC                                                                      & -2.4783      & -3.5265      \\
TNC/\textit{n}                                                           & -0.0826      & -0.0691      \\
Inhibition efficiency (in 0.5 mol·dm\textsuperscript{-3} H2SO4) [17], \% & 86,55        & 56,05        
\end{longtblr}
...
\fig[0.6\textwidth]{c/image17}[Fig. 3 - Side and top views of the most appropriate conﬁgu­ration for adsorption of SB1 (A) and SB2 (B) mole­cules on the Fe (110) surface obtained by MD simulations]
...
\fig[0.6\textwidth]{c/image18}[Fig.4 - Proposed adsorption mechanism for SB1 and SB2 on the Fe (110) surface in accordance with calculated descriptors]
...
Uralsk, Kazakhstan, e-mail: darupandandtary@gmail.com;

Kozhevnikova E.A. - student, M. Utemisov West Kazakhstan University,
Uralsk, Kazakhstan, e-mail: katerinakozevnikova853@gmail.com.
...
М.Утемисова, Уральск, Казахстан, e-mail: darupandandtary@gmail.com;
